...
if __name__ == "__main__":
    ...
    for i, (method, dataset) in enumerate(product(methods, datasets)):
        ...
        for tf in [True, False]:
            ...
            ax.plot(angles, values, "o-", color=color, clip_on=False, zorder=10, label=label)
            ax.fill(angles, values, alpha=0.25, color=color)
        ax.set_thetagrids(np.rad2deg(angles[:-1]), [expmet4show[col] for col in exp_cols])  # theta軸めもり
        ax.set_rlim(0, 1)  # r軸の範囲
        ax.set_rgrids(np.linspace(0, 1, num=5), ["0", "", "0.5", "", "1.0"], angle=30)  # r軸めもり

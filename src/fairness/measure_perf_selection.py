...
if __name__ == "__main__":
    ...
    for mi, method in enumerate(methods):
        for di, ds in enumerate(datasets):
            ...
            test_rep = f"{ds}-repair-test.csv"
            test_bre = f"{ds}-break-test.csv"
            # 必要なデータセットをロードする
    ...
    np.savetxt(os.path.join(save_dir, f"selection_results_fairness.csv"), res_arr, delimiter=",", fmt="%s")
...
from lib.util import dataset_type
...
num_folds = 5 # repair-break modelsを作る際のfolds
# 説明変数の列名のリスト
...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset", choices=DS_LIST, help=f"dataset name ({DS_LIST})")
    parser.add_argument("method", choices=METHOD_LIST, help=f"dataset name ({METHOD_LIST})")
    ...
    log_file_name = f"{ds}-fairness-{this_file_name}"
    ...
    is_binary = True
    nn_folds = 10 if ds in ["census", "bank"] else 5
    inv_nn_folds = 1 / nn_folds
        
    # rb dataset のdir
    preprocessed_rb_dir = f"/src/src/fairness/preprocessed_rb_datasets/{ds}/{method}/"
    # raw_rb_dirのファイルを取得
    rb_files = [f for f in os.listdir(preprocessed_rb_dir) if f.endswith(".csv")]
    # 上記ファイル名の最後の_の前の部分を取得しsetにする
    file_names_set = set([f.split("-", 1)[0] for f in rb_files])

    # 実行時間保存用df
    time_df = pd.DataFrame(columns=["rb", "fit_or_inf", "lr", "lgb", "rf"])

    # rb modelのdir
    save_dir = f"/src/src/fairness/repair_break_model/{ds}/{method}/"
    os.makedirs(save_dir, exist_ok=True)

    for division in file_names_set:
        for rb in ["repair", "break"]:
            logger.info(f"method = {method}, division = {division}, repair or break = {rb}")
            # 目的変数の列名
            obj_col = "repaired" if rb == "repair" else "broken"
            # モデル作成時の目的関数
            scoring = "recall" if rb == "break" else "f1"
            # 対象のcsvファイル名
            train_csv_name = f"{division}-{rb}-trainval.csv"
            test_csv_name = f"{division}-{rb}-test.csv"
            # 必要なデータセットをロードする
            train_ds_dir = os.path.join(preprocessed_rb_dir, train_csv_name)
            test_ds_dir = os.path.join(preprocessed_rb_dir, test_csv_name)
            ...
            verbose_level = 1
            # 結果保存用のcsvファイル名
            train_res_filename = f"{division}-{rb}-train.csv"
            test_res_filename = f"{division}-{rb}-test.csv"
            ...
            for cname in ["lr", "lgb", "rf"]:
                ...
                # 対象のpklファイル名
                model_filename = f"{division}-{rb}-{cname}.pkl"
        ...
        # 実行時間を保存
        time_df.to_csv(f"/src/experiments/time_for_repair_break_model-fairness-{division}-{method}.csv", index=False)

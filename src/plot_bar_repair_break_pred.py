    ...
    for rb in ["repair", "break"]:
        ...
        for i, method in enumerate(methods):
            ...
            for dataset in datasets:
                ...
                new_rows = np.array([[dataset4show[dataset]]*len(used_metrics), used_metrics, max_arr]).T
                new_entry = pd.DataFrame(data=new_rows, columns=["dataset", "metric", "val"])
                df = pd.concat([df, new_entry])
            df["val"] = df["val"].astype("float")
            # 全データセット終わったら描画する
            palette = sns.color_palette(n_colors=len(used_metrics))
            plt.xticks(rotation=45)
            sns.barplot(data=df, x="dataset", y="val", hue="metric", palette=palette, ax=ax)
        ...
        plt.legend(bbox_to_anchor=(-0.7, -0.25), loc="upper center", ncol=len(used_metrics))
        fig.tight_layout()
        fig.subplots_adjust(left=0, right=1, bottom=0, top=1, hspace=0.1, wspace=0.2)
        # plt.show()
        plt.savefig(f"./bar_pred_perf_{rb}.pdf", bbox_inches="tight")
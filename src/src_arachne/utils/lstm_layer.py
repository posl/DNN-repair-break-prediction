...
class LSTM_Layer(object):
    ...
    def gen_lstm_layer_from_another(self, prev_output):
        ...
        """
        # from tensorflow.keras.layers import LSTM, CuDNNLSTM # CuDNNLSTMインポートできない
        ...
        if len(self.init_lstm_layer.get_weights()) == 3:
            kernel_w, recurr_kernel_w, bias = self.init_lstm_layer.get_weights()
        else:
            kernel_w, recurr_kernel_w = self.init_lstm_layer.get_weights()

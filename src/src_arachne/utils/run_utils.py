...
def run_model(mdl, X, y, is_multi_label=True, ret_raw=False):
    ...
    predcs = mdl.predict(X)  # NOTE: softmax取る前の値(最後の全結合層の出力値)が入ってる. Xは複数のデータなので, (#data, #label)の形状で
    ...
    return aft_pred_df, predcs


def gen_and_run_model(
    mdl, path_to_patch, X, y, num_label, has_lstm_layer=False, is_multi_label=True, need_act=False, batch_size=None
):
    """
    ** this is the part that should be fixed (this is a temporary fix)
    ...
    if not has_lstm_layer:
        ...
        predictions = kfunc_util.compute_kfunc(
            k_fn_mdl_lst, formated_y, [patch[idx] for idx in indices_to_tls], batch_size=batch_size
        )[0]
    else:
    ...
    if is_multi_label:
        # たぶんpredictionsにはsoftmaxとる前の値が入っている
        # softmaxとらなくても大小関係は変わらないのでこれでも問題はない

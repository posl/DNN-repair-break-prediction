...
"""
import os
from src_arachne.utils import model_util
...
def build_mdl_lst(org_mdl, prev_out_shape, indices_to_tls):
    ...
    mdl = Model(inputs=model_input, outputs=network_dict["new_output_tensor_of"][last_layer_name])  # this is a constant
    return mdl
...
def build_k_frame_model(mdl, X, indices_to_tls, act_func=None):
    ...
    for idx_to_l in range(min_idx_to_tl, num_layers):
        ...
        # print(network_dict)
        # Determine input tensors
        # ここで最初のレイヤを指定するとnetwork_dict['input_layers_of'][layer.name]でKeyErrorになる
        layer_input = [
        ...
        if idx_to_l in indices_to_tls:
            ...
            if is_target(l_class_name, targeting_clname_pattns):
                if model_util.is_FC(l_class_name):
                    ...
                    x = tf.add(
                        tf.tensordot(layer_input, t_w, [[len(layer_input.shape) - 1], [0]]), t_b, name=layer_name
                    )

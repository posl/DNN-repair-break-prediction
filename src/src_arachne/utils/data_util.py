...
def load_data(which, path_to_data, is_input_2d=False, path_to_female_names=None, path_to_namedir=None):
    import os
    ...
    if which == "lfw":
        ...
        names_in_train = sorted(get_lfw_data(path_to_namedir, is_train=True)["name"])
        names_in_test = sorted(get_lfw_data(path_to_namedir, is_train=False)["name"])

        trainloader = get_LFW_loader(
            image_path=path_to_data, split="train", batch_size=1, path_to_female_names=path_to_female_names
        )
        testloader = get_LFW_loader(
            image_path=path_to_data, split="test", batch_size=1, path_to_female_names=path_to_female_names
        )
        ...
        if which == "fashion_mnist":
            trainset = torchvision.datasets.FashionMNIST(
                root=path_to_data, train=True, download=True, transform=transforms.ToTensor()
            )
            testset = torchvision.datasets.FashionMNIST(
                root=path_to_data, train=False, download=True, transform=transforms.ToTensor()
            )
        else:
            trainset = torchvision.datasets.CIFAR10(
                root=os.path.join(path_to_data, "train"), train=True, download=True, transform=transforms.ToTensor()
            )
            testset = torchvision.datasets.CIFAR10(
                root=os.path.join(path_to_data, "test"), train=False, download=True, transform=transforms.ToTensor()
            )
...
def get_misclf_indices_balanced(df, idx=0):
    ...
    # dfをtrue, pred列ごとにカウントをまとめ、カウントの合計値を出力
    # print(df.groupby(["true","pred"]).size().sum())
    ...
    for misclf_type in misclf_types:
        ...
        indices_to_misclf = df.loc[(df.true == true_label) & (df.pred == pred_label)].index.values
...
def get_balanced_dataset(pred_file, top_n, idx=0):
    ...
    print(f"misclf_df.shape: {misclf_df.shape}")
    ...
    print(f"indices_to_corr.shape: {indices_to_corr.shape}")
    # あってたデータをtargetとevalの半分ずつに分ける
    ...
    if top_n >= len(sorted_keys):
        msg = "{} is provided when there is only {} number of misclfs".format(top_n, len(sorted_keys))
        assert False, msg
    else:
        misclf_key = sorted_keys[top_n]
        # topn番目に多いmisclfのindexを取得

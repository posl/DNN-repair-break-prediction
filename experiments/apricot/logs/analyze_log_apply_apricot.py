...
if __name__ == "__main__":
    ...
    if is_parse_only:
        ...
        output_lines = []
        for line in lines:
            if "Start apricot_repair.py" in line or "Total execution time" in line or "saved" in line:
        ...
        t = []  # foldsごとの時間
        pat1 = r"Total execution time: (\d+\.\d+)"  # 実行時間の部分を取り出す正規表現
        pat2 = r".+fold-(\d).pt"  # fold数の部分を取り出す正規表現
